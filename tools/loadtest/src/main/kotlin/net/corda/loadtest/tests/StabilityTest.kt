    ...
    fun crossCashTest(replication: Int) = LoadTest<CrossCashCommand, Unit>(
            ...
            generate = { _, _ ->
                val payments = simpleNodes.flatMap { payer -> simpleNodes.map { payer to it } }
                        ...
                        .map { (payer, payee) -> CrossCashCommand(CashFlowCommand.PayCash(Amount(1, USD), payee.mainIdentity, anonymous = true), payer) }
            ...
            generate = { _, _ ->
                ...
                val generateIssue =
                        simpleNodes.map { issuer ->
                            SelfIssueCommand(CashFlowCommand.IssueCash(Amount(100000, USD), OpaqueBytes.of(0), issuer.mainIdentity, notary.info.notaryIdentity, anonymous = true), issuer)
                        }
