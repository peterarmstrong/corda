...
fun ServiceHub.fillWithSomeTestLinearStates(numberToCreate: Int,
    ...
    val transactions: List<SignedTransaction> = (1..numberToCreate).map {
        ...
        val signatureMetadata = SignatureMetadata(myInfo.platformVersion, Crypto.findSignatureScheme(DUMMY_NOTARY_KEY.public).schemeNumberID)
        return@map signInitialTransaction(dummyIssue).withAdditionalSignature(DUMMY_NOTARY_KEY, signatureMetadata)
    }
...
fun <T : LinearState> ServiceHub.consume(states: List<StateAndRef<T>>, notary: Party) {
    ...
    states.forEach {
        val consumedTx = TransactionBuilder(notary = notary).let { builder ->
            builder.addInputState(it)
            this.signInitialTransaction(builder, notary.owningKey)
        }
...
fun <T : LinearState> ServiceHub.consumeAndProduce(stateAndRef: StateAndRef<T>, notary: Party): StateAndRef<T> {
    ...
    // Create a txn consuming different contract types
    val producedTx = TransactionBuilder(notary = notary).let { builder ->
        builder.addOutputState(DummyLinearContract.State(linearId = stateAndRef.state.data.linearId,
...
fun ServiceHub.consumeCash(amount: Amount<Currency>, to: Party = CHARLIE, notary: Party): Vault.Update<ContractState> {
    ...
    val spendTX = TransactionBuilder(notary).let { builder ->
        vaultService.generateSpend(builder, amount, to)
        this.signInitialTransaction(builder, notary.owningKey)
    }
